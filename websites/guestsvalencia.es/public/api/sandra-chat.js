function getModelForRole(role) {
  ...
  export default async function handler(req, res) {
    ...
    try {
      ...
      const body = req.body || {};
      const message = (body.message || body.mensaje || "").toString().trim();
      const role = (body.role || body.rol || "visitor").toString().trim().toLowerCase();
      ...
      const systemPrompt =
        "Eres Sandra, una asistente de reservas para Guests Valencia. Responde en español con tono cálido,
  profesional y directo. " +
        "Si el usuario pide reservar: pide fechas (llegada, salida), nº de huéspedes y presupuesto. Sé breve y
  útil.";
      ...
      if (!resp.ok) {
        ...
        console.error(`[${debugId}] OpenAI error: status=${resp.status} elapsed=${elapsed}ms body=${errText}`);
      ...
      try { data = await resp.json(); }
        ...
        return res.status(502).json({ error: "Bad response from OpenAI", debugId });
      }
      ...
      if (!reply) {
        ...
        return res.status(200).json({
          ...
          respuesta: "Lo siento, ahora mismo no puedo responder.",
          modelUsed: model,
          role,
          debugId
        });
      ...
      return res.status(200).json({
        reply,
        respuesta: reply, // Para compatibilidad con tu frontend
        modelUsed: model,
        role,
        debugId
      });
