...
exports.handler = async (event)=>{
  ...
  let orderId; try{ orderId=JSON.parse(event.body||"{}").orderId }catch{ return cors({ok:false,error:"Bad JSON"},400) }
  if(!orderId) return cors({ok:false,error:"orderId required"},400);
  try{ const t = await token();
    const res = await fetch(`${BASE}/v2/checkout/orders/${orderId}/capture`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+t}});
